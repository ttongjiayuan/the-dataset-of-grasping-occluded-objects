...
def check_push_clutter_degree(prev_best_pix_ind, prev_push_end_pix_yx, prev_depth_heightmap,valid_depth_heightmap,num_depth,prev_num_depth):
    add_depth_sum=np.sum(valid_depth_heightmap)-np.sum(prev_depth_heightmap)
    add_num_depth=num_depth-prev_num_depth
    print(add_depth_sum)
    print(add_num_depth)
    return [add_depth_sum,add_num_depth]
