...
#
                    ...
                    # Actor
                    if not is_testing and trainer.iteration < 200000:
    ...
    def reposition_objects():
        datatjy = random.randint(0, 9)
        if not is_testing:
            config_file = 'simulation/preset/coordination-0' + str(datatjy) + '.txt'
        else:
            config_file=args.config_file
        robot.sim_read_config_file(config_file)
        robot.restart_sim()
        robot.add_objects()
        time.sleep(15)
        grasp_fail_count[0] = 0
    ...
    while True:
        ...
        if (len(mask_heightmaps['names']) == 0 and not test_target_seeking) or motion_fail_count[0] >= max_motion_onecase:
            ...
            if is_testing:
                trainer.model.load_state_dict(torch.load(critic_ckpt_file))
            continue
        if (len(mask_heightmaps['names']) > 0 ) or motion_fail_count[0] >= max_motion_onecase:
            # Restart if no targets detected
            next_sence = next_sence+1
            if next_sence>1:
                reposition_objects()
                next_sence=0
                target_name = None
        ...
        if len(mask_heightmaps['names']) == 0 and test_target_seeking:
            ...
            if target_name in mask_heightmaps['names']:
                ...
                print(len(mask_heightmaps['names']))
        ...
        for i in range(224):
            for j in range(224):
                if (valid_depth_heightmap[i][j] > 0.01):
        ...
        if 'prev_color_img' in locals():
            ...
            if prev_primitive_action == 'push':
                ...
                nonlocal_variables['clutter_degree'] = utils.check_push_clutter_degree(prev_best_pix_ind,
                                                                                   prev_push_end_pix_yx,
                                                                                   prev_valid_depth_heightmap,
                                                                                   valid_depth_heightmap, num_depth,
                                                                                   prev_num_depth)
            ...
            if not robot.objects_reset:
                ...
                if not prev_target_grasped:
                    ...
                    margin_increase_val =np.array(100)
            ...
            label_value, prev_reward_value = trainer.get_label_value(
                ...
                color_heightmap, valid_depth_heightmap, target_mask_heightmap, target_found, nonlocal_variables['clutter_degree'])
            #print(label_value,1111)
            trainer.target_found_log.append([target_found])
            logger.write_to_log('target_found', trainer.target_found_log)
            trainer.label_value_log.append([label_value])
            logger.write_to_log('label-value', trainer.label_value_log)
            trainer.reward_value_log.append([prev_reward_value])
            logger.write_to_log('reward-value', trainer.reward_value_log)
            trainer.xiangsu_log.append(nonlocal_variables['clutter_degree'])
            logger.write_to_log('xiangsu', trainer.xiangsu_log)

            if prev_primitive_action == 'push':
                trainer.executed_action_log.append(
                    [0, prev_best_pix_ind[0], prev_best_pix_ind[1],
                     prev_best_pix_ind[2]])  # 0 - push
            elif nonlocal_variables['primitive_action'] == 'grasp':
                trainer.executed_action_log.append(
            ...
            trainer.predicted_value_log.append([np.max(pre_push_value)])
            logger.write_to_log('predicted-value', trainer.predicted_value_log)
            # Backpropagate
            # regular backprop
        ...
        if nonlocal_variables['seeking_target']:
            ...
            prev_mask_heightmaps = mask_heightmaps.copy()  # 1
            prev_target_mask_heightmap = target_mask_heightmap.copy()  # 1
            ...
            prev_margin_occupy_ratio = np.array(100)
            prev_num_depth = num_depth
            pre_push_value = push_predictions
            # prev_color_img = color_img.copy()
            # prev_depth_img = depth_img.copy()
            # prev_color_heightmap = color_heightmap.copy()
            # prev_depth_heightmap = depth_heightmap.copy()
...
if __name__ == '__main__':
    ...
    # --------------- Object options ---------------
    parser.add_argument('--config_file', dest='config_file', action='store', default='simulation/preset/coordination-14.txt')#exploration-00,coordination-00
    ...
    parser.add_argument('--max_test_trials', dest='max_test_trials', type=int, action='store', default=20, help='number of repeated test trials')

    # ------ Pre-loading and logging options ------
    parser.add_argument('--load_ckpt', dest='load_ckpt', action='store_true', default=True)
    parser.add_argument('--critic_ckpt', dest='critic_ckpt', action='store',default='/home/tongjiayuan/tjy/gnn-invisible/logs/2021-12-08.21:30:10/critic_models/critic-001800.pth')
    parser.add_argument('--coordinator_ckpt', dest='coordinator_ckpt', action='store', default='/home/tongjiayuan/tjy/invisible1/logs/2021-11-29.23:23:21/coordinator_models/coordinator-000000.pth')
    parser.add_argument('--continue_logging', dest='continue_logging', action='store_true', default=False)
    parser.add_argument('--logging_directory', dest='logging_directory', action='store',default='/home/tongjiayuan/tjy/invisible1/logs/2021-12-01.20:35:18')
